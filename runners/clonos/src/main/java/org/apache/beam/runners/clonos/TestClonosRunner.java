...
/** Test Flink runner. */
public class TestClonosRunner extends PipelineRunner<PipelineResult> {

  private ClonosRunner delegate;

  private TestClonosRunner(FlinkPipelineOptions options) {
    options.setRunner(TestClonosRunner.class);
    options.setShutdownSourcesOnFinalWatermark(true);
    if (options.getParallelism() == -1) {
      // Limit parallelism to avoid too much memory consumption during local execution
      options.setParallelism(1);
    }
    this.delegate = ClonosRunner.fromOptions(options);
  }

  public static TestClonosRunner fromOptions(PipelineOptions options) {
    FlinkPipelineOptions flinkOptions = options.as(FlinkPipelineOptions.class);
    return new TestClonosRunner(flinkOptions);
  }

  public static TestClonosRunner create(boolean streaming) {
    ...
    return TestClonosRunner.fromOptions(flinkOptions);
  }
