 ...
 * configuration.
 */
public class ClonosRunner extends PipelineRunner<PipelineResult> {

  private static final Logger LOG = LoggerFactory.getLogger(ClonosRunner.class);
   ...
   */
  public static ClonosRunner fromOptions(PipelineOptions options) {
    ...
    if (flinkOptions.getFilesToStage() == null) {
      flinkOptions.setFilesToStage(
          detectClassPathResourcesToStage(ClonosRunner.class.getClassLoader(), options));
      LOG.info(
    ...
    return new ClonosRunner(flinkOptions);
  }

  protected ClonosRunner(FlinkPipelineOptions options) {
